...
<div id="content" style="display:none;">
	<button type="button" class="btn btn-primary btn-sm"
		data-toggle="modal" data-target="#myModal">增加角色</button>
		...
		<tbody>
			<c:forEach items="${roles}" var="role">
				<tr>
					<td>${role.name}</td>
					<td><a
						data-href="<c:url value='/role/del.html?id=${role.id}'/>"
						class="deleteBtn">删除</a> <a href="<c:url value=''/>">修改</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
$(function() {
	...
	$.ajax({
		...
		success:function(data){
			...
			tab.row.add([
				data.name,
				"<a onclick='del(this);' data-href='<c:url value='/role/del.html?id=' />"+data.id+"'>删除 </a><a href='#'>修改</a>"
			]).draw();
			$('#newRow').attr("data-href","<c:url value='/role/del.html?id=' />"+data.id);					
		}
	});
});
});
